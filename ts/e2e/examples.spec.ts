...
test.describe("3D Orbital Viewer Example", () => {
  ...
  test("has 10 preset buttons", async ({ page }) => {
    await page.goto("/examples/orbital-3d.html");
    const buttons = page.locator(".preset-btn");
    ...
    await expect(buttons.nth(6)).toContainText("木星捕獲");
    await expect(buttons.nth(7)).toContainText("土星リング");
    await expect(buttons.nth(8)).toContainText("天王星接近");
    await expect(buttons.nth(9)).toContainText("地球到着");
  });
