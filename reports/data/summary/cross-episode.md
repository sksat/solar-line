...
```3d-viewer:
{ "scene": "full-route", "scenes": ["full-route", "jupiter-capture", "saturn-ring", "uranus-approach"], "height": 500, "caption": "ケストレル号の全航路（火星→木星→土星→天王星→地球）を3次元で可視化。木星捕獲シーンではペリジュピター捕獲（1.5 RJ）とIF高高度捕獲の比較を、土星リングシーンではリング面通過の幾何学を、天王星接近シーンでは自転軸傾斜98°の影響を示す。マウスドラッグで回転、スクロールでズーム。タイムスライダーで約124日間の航行をアニメーション再生可能。" }
```
