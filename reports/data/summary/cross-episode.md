...
```3d-viewer:
{ "scene": "full-route", "scenes": ["full-route", "saturn-ring", "uranus-approach"], "height": 500, "caption": "3Dインタラクティブビューア — マウスドラッグで回転、スクロールでズーム。全航路シーンではタイムスライダーで約124日間の航行をアニメーション再生できます。" }
```
