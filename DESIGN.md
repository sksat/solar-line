...
解析には Typescript や Rust を用いること。Rust を用いた解析をした場合は、レポートには手元で動作させた解析結果を貼るか、wasm にビルドしてレポートでの再現が可能になるようにすること。Rust の依存ライブラリについて、wasm ビルドするので no_std 対応しているもの（nalgebra 等）は使ってよい。計算の精度確認には、信頼できるライブラリ実装をオラクルとしたテストを組むことで、計算内容を保証しつつ wasm ビルドできるシンプルな構成を維持する。
...
Web ページとしての成立性検証（Playwright E2E: レンダリング、リンク整合性）と記事内容の検証テストは論理的に意味が異なるもの。後者はテストの仕組みを既存のツール（Playwright など）に乗っかりにくいため、このプロジェクト内でメンテナンスする必要がある。

記事内の検証を「テスト」として扱い、TDD 的アプローチによる記事編集・自動テストによる記事の内容が検証されている状態の維持を行う:
- レポート内の数値が計算結果と一致しているか
- エピソード間のパラメータ引用が整合しているか
- 台詞引用がトランスクリプションデータと一致しているか
- 統計値（テスト数、タスク数等）が最新か
- `npm test` で高速に実行可能な TS ユニットテストとして実装すること
...
DAG は単に並べて可視化するだけでは分かりにくい。解きほぐして表示しないと、どんな依存関係があって、何を変更したら何を直さないといけなくなるのかがわかりにくい。
DAG の分析を Rust でモデリングすることで、可視化時に WASM 呼び出しでブラウザ上でリアルタイム分析が可能になる。

## インタラクティブ時系列グラフ

インタラクティブな時系列グラフには uPlot を使用すること。軽量・高速な Canvas ベースのチャートライブラリで、推力プロファイルや放射線量推移などの可視化に適している。

## データ管理

ブラウザ上のデータ管理には DuckDB-WASM を使用すること。JSON データの SQL クエリ、集計、フィルタリングが可能になる。uPlot との連携でクエリ結果を直接グラフ化できる。

## 他船舶分析とインフラ分析の分離

他船舶（保安艦隊、大型船、保安艇、商船群）の分析と太陽系インフラ（ステーション、航路誘導網、ガバナンス機構）の分析は別レポートに分けること。他船舶の分析には軌道遷移分析（ΔV、所要時間、遷移図）を含め、各話分析にも反映する。

## 考証の観点

「科学的精度」という単一軸でレポートをまとめるとスコープがぼやける。現実の物理法則との整合性、設定パラメータの出典とその確からしさ、物語内の一貫性はそれぞれ異なる問題であり、混同しないこと。特に、現実の観測値と作品内パラメータの一致は「精度」ではなく、考証（出典とその信頼性）の問題として扱う。

## レポートレビュー体制

レポートのレビューは、gpt-5.2 や Task agent などの **別コンテキスト** を持ったエージェントに行わせること:
- **ペルソナ設定**: SOLAR LINE に対する事前知識がなく、SF や軌道力学に関心はあるが詳しくはない読者
- **ドラフトレビュー**: コードベースにアクセスできる状態で、データの整合性や分析の論理を検証
- **最終レビュー**: 一時ディレクトリでコードベースを見せない状態で起動し、デプロイ済みの GitHub Pages にアクセスさせて読者体験を検証

## 軌道遷移図のバーン位置整合性

軌道遷移アニメーションにおける加速点・減速点の位置は、アニメーションの船の位置と物理的に正しい位置を一致させること。brachistochrone の中間点フリップ、Hohmann の出発/到着バーン、フライバイ近点でのバーンなど、遷移種別ごとに適切な位置を設定する。

## 質量時系列分析

コンテナ分離、推進剤消費、損傷による質量変化を時系列で分析・可視化すること。複数パターン（公称値ベース、299t上限ベース、推進剤最適化）を想定し、比較可能な形で表示する。

## キャラクター名の扱い

原作で明示されていないキャラクター名を勝手に命名しないこと。名前が不明な場合は役割で呼称する（例: 「船乗り」）。

## 内部ドキュメントの公開

current_tasks/ や ADR などのプロジェクト内部ドキュメントも GitHub Pages サイト上で閲覧可能にすること。

## ADR の記録品質

ADR には Alternatives Considered（検討した他の選択肢）と Assumptions（意思決定の前提条件）のセクションを含めること。意思決定を後で変更する際に、どんな前提でその決定が行われ、他にどんな選択肢があったのかの記録が重要になる。

## 軌道遷移図の説明

軌道遷移図にはタイトルだけでなく、なんのための分析で、何に注目すべきで、何が読み取れるかを記述する description を含めること。

## 誤差範囲の可視化

時系列グラフや軌道遷移図において、誤差の範囲も可視化すること。不確実性バンド、軌道バリエーションの重ね描き、誤差楕円などを用いて、計算結果の信頼度を視覚的に表現する。

## CI の安定性

CI はほぼ常に pass を維持できるワークフローとすること。コミット前にローカルで最低限のチェック（cargo fmt --check, cargo clippy, npm run typecheck）を実施し、CI を壊すコミットを push しない。

## サブエージェントのモデル選択

Haiku は性能が不十分であるため使用しない。サブエージェントにはデフォルトで Sonnet を使用し、複雑なタスクには Opus を使用する。
