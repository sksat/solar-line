...
- **G-tolerance as SF premise**: Human G-tolerance is never explicitly discussed in the series, suggesting it has been solved by some advanced technology. When analyzing mass discrepancies or acceleration profiles, assume SF-level G-tolerance countermeasures (inertial dampers, etc.) exist as a default premise. If allowable acceleration is higher than modern human limits, the mass/thrust relationship changes — explore this parameter space. Note: structural sections and habitation sections of the ship should be analyzed separately — structural components may withstand far higher G-loads than crew quarters.
