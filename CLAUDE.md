...
SOLAR LINE 考証 — astrodynamics analysis of the SF anime series "SOLAR LINE" from『良いソフトウェアトーク劇場』by ゆえぴこ.
...
- **Orbital animation arrival alignment**: After a transfer animation completes, the ship must arrive at the correct destination body position. Verify that animated transfer endpoints match the destination body's orbital position at the arrival time. This is a known recurring issue — test and fix systematically.
- **Math rendering regression tests**: KaTeX math rendering (`$...$` inline, `$$...$$` display) occasionally breaks. Add TDD-based regression tests to catch and prevent rendering issues. Maintain math rendering correctness through automated tests.
...
- **G-force category separation**: Distinguish between two fundamentally different categories of G-force:
  1. **居住G（habitation G）**: The gravitational environment where characters live and have adapted (e.g., 0.8G for outer-garden residents, 1G on Earth). This is a **worldbuilding/setting parameter** describing long-term adaptation.
  2. **推進G（propulsion G）**: Acceleration/deceleration forces from engine thrust during orbital transfers (e.g., 2-3G brachistochrone). This is a **physics analysis parameter**.
  These must NOT be conflated. In SF works, propulsion G is conventionally ignored or implicitly handled — characters experience acceleration without visible distress, and strict analysis (requiring rotating ships, gel couches, etc.) produces uncinematic results that SF intentionally avoids. The fact that a character can't tolerate 1G habitation gravity does NOT imply they'd suffer equally from 2-3G propulsion acceleration — SF treats these differently as a genre convention. Structural sections and habitation sections of the ship should also be analyzed separately — structural components may withstand far higher G-loads than crew quarters.
...
- **EP03 captive identity**: The person captured by Earth military in EP03 scene-05 (interrogation/1G scene) may NOT be きりたん — verify by re-watching the scene before attributing dialogue. Do not assume the captive's identity without visual confirmation.
...
- **AI-generated disclaimer and spoiler warning**: The site must prominently display at the top of every page (or at minimum the landing page) that the analysis is mostly AI-generated, and a strong spoiler warning for the series. These notices must be immediately visible, not buried in footnotes.
- **Unapproved ADR for owner decisions**: When a decision requires project owner approval, create an ADR with "Proposed" status (not "Accepted"). This makes pending decisions visible and trackable. Do not assume owner approval — wait for explicit confirmation before changing status to "Accepted".
